...
pygame.display.set_caption('Simulation 3 Robots a 2 Roues et Gestion des couleurs')
...
# LSB 15.32 représente la vitesse max de rotation des roues pour notre robot (en réalité c'est la roue la moins rapide de mon robot 2 CAN par robot est meilleur)
quantum = 15.32/resol 
...
x=[600,600,600] # Position de départ
y=[600,600,600]
x1 = [0,0,0]  # Variable pour enregister l'ancienne position
y1 = [0,0,0]
theta = [0,61,2] # Angle de rotation de chaque robot au départ
r = 4 # rayon du robot
L = 15 # longeur du robot

# les vitesses sont cappé à 15.32
wg = [15,15,15] 
wd = [15,15,15]
owG = [0,0,0] # Variable pour enregister l'ancienne vitesse de rotation
owD = [0,0,0]
...
i = [0,0,0]  # I
d = [0,0,0]  # D
uG = [0,0,0] # Sortie du moteur gauche vers le système (::setU())
uD = [0,0,0] # Sortie du moteur droit vers le système (::setU())
...
while(boolean):
    ...
    if cpt%75 == 0 and stopB:
        ...
        ################################################
        # Modification des valeurs moteurs wd et wg pour roue gauche et droite
        # pour Robot 1
        ...
        # Animation
        #pygame.draw.circle(SCREEN,(0,0,0),(x,y),2) # Affichage de point pour chaque itération
...
# Prevent error after program ends
boolean = True
while(boolean):
...
# On remarquera des valeurs abérante pour le PID qui ne subit pas la rétroaction par absence de valeurs réelles
